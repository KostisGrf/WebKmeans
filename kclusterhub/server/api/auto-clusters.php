...
if(empty($categorical_columns) && empty($numerical_columns)){
    header("HTTP/1.1 400 Bad Request");
    print json_encode(['errormesg'=>"The columns field cannot be empty"]);
    exit();
}
...
$py_return=shell_exec("python3 ../python/auto_clusters.py $path $colums_string $clusters $ext $path_to_save  2>&1");



if(filter_var($py_return,FILTER_VALIDATE_BOOL)){
    $error_response = array(
        "error" => array(
          "message" => "There was an error during initialization.Please try running the algorithm again with fewer clusters."
        )
      );
